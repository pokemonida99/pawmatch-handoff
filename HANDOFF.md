...
### 🆕 v2 報告重跑後的新待辦（2026-07-09，Fable 5 重新分析產出，依據見 USABILITY-REPORT.md Part 6）
- [ ] **P0 貓卡可掃描事實列**：地區＋親貓／親狗／親幼兒章（B 受訪者發文格式證詞；P4 persona 在貓卡層流失）
- [ ] **P1 雙貓成家**：「我們是一起的」標記＋測驗「一隻或兩隻」題＋社會化論述轉賣點（A/B 不拆送實務）
- [ ] **P1 回報提醒時間衰減**：首 3 月照常→季報→年度近況卡（C：「過一年兩年就不太回報」）
- [ ] **P2 校友迴路**：相簿卡標「這是我第 N 隻」＋帶朋友入口（A 的 1→6 隻重複認養）
- [ ] **P2 列表篩選器**：地區／親善／年齡（60 隻規模前置準備）
- 反建議（不要做）：公開黑名單（誹謗風險）、把測驗變成審核（A 型中途進場前提）
- GTM 註記：FB 懲罰外部連結——需要無連結的原生分享圖卡＋QR code
